...
internal class AppointmentService : IAppointmentService
{
    ...
    public async Task SendHubJob(DateOnly date, string patient, string doctor, string role)
    {
        try
        {
            ...
            foreach (var staff in staffUser)
            {
                users.Add(staff.Id);
            }
            users.Add(patient);
    ...
    public async Task VerifyAndFinishBooking(PayAppointmentRequest request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            _jobService.Enqueue(
                    () => SendHubJob(appointment.AppointmentDate, patientId.UserId, doctor.DoctorId, currentUserRole));
            await DeleteRedisCode();
        }
        catch (Exception ex)
        {
    ...
    public async Task RescheduleAppointment(RescheduleRequest request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var doctor = await _db.DoctorProfiles.FirstOrDefaultAsync(p => p.Id == appointment.DentistId);
            _jobService.Enqueue(
                () => SendHubJob(appointment.AppointmentDate, patient.UserId, doctor.DoctorId, user_role));
            await DeleteRedisCode();
        }
        catch (Exception ex)
        {
    ...
    public async Task CancelAppointment(CancelAppointmentRequest request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var doctor = await _db.DoctorProfiles.FirstOrDefaultAsync(p => p.Id == appoint.DentistId);
            _jobService.Enqueue(
                () => SendHubJob(appoint.AppointmentDate, patient.UserId, doctor.DoctorId, user_role));
            await DeleteRedisCode();
        }
        catch (Exception ex)
        {
    ...
    public async Task ScheduleAppointment(ScheduleAppointmentRequest request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var doctor = await _db.DoctorProfiles.FirstOrDefaultAsync(p => p.Id == appointment.DentistId);
            _jobService.Enqueue(
                () => SendHubJob(appointment.AppointmentDate, patient.UserId, doctor.DoctorId, currentUserRole));
            await DeleteRedisCode();
        }
        catch (Exception ex)
        {
    ...
    public async Task<List<TreatmentPlanResponse>> ToggleAppointment(Guid id, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (appointment.Status == AppointmentStatus.Confirmed)
            {
                ...
                var doctors = _userManager.FindByIdAsync(dprofile.DoctorId!).Result;
            ...
            var doctor = await _db.DoctorProfiles.FirstOrDefaultAsync(p => p.Id == appointment.DentistId);
            _jobService.Enqueue(
                () => SendHubJob(appointment.AppointmentDate, patient.UserId, doctor.DoctorId, _currentUserService.GetRole()));
            await DeleteRedisCode();
        }
        catch (Exception ex)
        {
    ...
    public async Task HandlePaymentRequest(PayAppointmentRequest request, CancellationToken cancellationToken)
    {
        try
        {
            if (request.IsPay && (request.Method == Domain.Payments.PaymentMethod.Cash))
            {
                ...
                var doctor = await _db.DoctorProfiles.FirstOrDefaultAsync(p => p.Id == query.Payment.Appointment.DentistId);
                _jobService.Enqueue(
                () => SendHubJob(query.Payment.Appointment.AppointmentDate, user.Id, doctor.DoctorId, _currentUserService.GetRole()));
                await DeleteRedisCode();
                _jobService.Enqueue(
    ...
    public async Task DoPaymentForAppointment(PayAppointmentRequest request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var doctor = await _db.DoctorProfiles.FirstOrDefaultAsync(p => p.Id == query.Payment.Appointment.DentistId);
            _jobService.Enqueue(
                () => SendHubJob(query.Payment.Appointment.AppointmentDate, patient.UserId, doctor.DoctorId, _currentUserService.GetRole()));
            await DeleteRedisCode();
            _jobService.Enqueue(
                () => SendInvoiceToMail(query.Payment.AppointmentId.Value, user));
        }
    ...
    public async Task<string> AddDoctorToAppointments(AddDoctorToAppointment request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            _jobService.Enqueue(
                () => SendHubJob(appoitment.Appointment.AppointmentDate, patient.UserId, doctor.DoctorId, _currentUserService.GetRole()));
            await DeleteRedisCode();
            return _t["Success"];
        }
        catch (Exception ex)
        ...
        try
        {
            ...
            var appointments = await appointmentsQuery
                .Select(appointment => new
                {
                    ...
                    Patient = _db.PatientProfiles.FirstOrDefault(p => p.Id == appointment.PatientId),
                    TreatmentPlan = _db.TreatmentPlanProcedures.IgnoreQueryFilters().FirstOrDefault(s => s.Id == appointment.PlanID),
                })
                .ToListAsync(cancellationToken);
            ...
            foreach (var a in appointments)
            {
                ...
                var sp = await _db.ServiceProcedures.IgnoreQueryFilters().Where(p => p.Id == a.TreatmentPlan.ServiceProcedureId)
                    .Select(s => new
                    {
